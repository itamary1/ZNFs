...
class EntrezClint:
    ...
    def get_prot_seq(self, accession_n):
        ...
        assert prot_seq[0] == "'" and prot_seq[-1] == "'", (
            accession_n + " protein seq didnt fully accomplished " + prot_seq
        )
...
class ZnfDomainsHits:
        ...
        for hit in c2h2_comped.finditer(seq):
            ...
            for area in area_comp.finditer(domain):
                ...
                if 2 < area.start() < 6:
                    ...
                    self.four_binding.append(
                        tuple([h_place - 7, h_place - 5, h_place - 4, h_place - 1])
                    )
    ...
    def get_statistcs(self):
        return {
            "num_of_domains": self.num_domains,
            "num_of_aminos": len(self.binding_aminos),
            "span": self.domains_span,
        }
    ...
    with open(parent_dir + "/domain_editing/ZNF_binding_edit.txt", "w") as outf:
        ...
        for index, gene in genes.iterrows():
            ...
            for index, site in g_sites.iterrows():
                ...
                assert orig_amino == prot_seq[amino_place], (
                    "orig: "
                    + orig_amino
                    + "seq :"
                    + prot_seq[amino_place]
                    + "\n"
                    + str(site)
                    + "\n\n"
                    + prot_seq
            ...
            gene_binding_aminos_rate = round(
                gene_bind_counter / hits_check.get_statistcs()["num_of_aminos"]
                if hits_check.get_statistcs()["num_of_aminos"] > 0
                else 0,
                4,
            )
            ...
            outf.write(
                "editing binding aminos rate expected: {}, results: {}\n".format(
                    gene_expected_aminos_rate, gene_binding_aminos_rate
                )
            )
            print(
                "editing binding aminos rate expected: {}, results: {}".format(
                    gene_expected_aminos_rate, gene_binding_aminos_rate
                )
            )
            ...
            # calculate expected rate
            gene_expected_in_domain_rate = hits_check.get_statistcs()["span"] / len(
                prot_seq
            )
            # calculate resulted rate
            ...
            # write output
            outf.write(
                "in ZNF-domain editing rate expected: {}, results: {}\n\n".format(
                    gene_expected_in_domain_rate, gene_in_domain_rate
                )
            )
            print(
                "in ZNF-domain editing rate expected: {}, results: {}".format(
                    gene_expected_in_domain_rate, gene_in_domain_rate
                )
            )
            print("")
        ...
        expected_aminos_count_total = sites.shape[0] * (
            total_binding_sites / proteins_len_all
        )
        outf.write(
            "total non-syn sites: {}, total sites in binding sites: {}, expected in binding site: {}\n".format(
                sites.shape[0], total_binds, round(expected_aminos_count_total, 4)
            )
        )
        print(
            "total non-syn non sites: {}, total sites in binding sits: {}, expected in binding site: {}".format(
                sites.shape[0], total_binds, round(expected_aminos_count_total, 4)
            )
        )
        outf.write(
            "total expected edit rate per point: {},total results: {}\n".format(
                round(expected_aminos_rate_total, 4),
                round(binding_aminos_rate_total, 4),
            )
        )
        print(
            "total expected edit rate per point: {},total results: {}".format(
                round(expected_aminos_rate_total, 4),
                round(binding_aminos_rate_total, 4),
            )
        )
        ...
        outf.write(
            "total sites in domains: {}, expected in domains: {}\n".format(
                total_in_domains, round(expected_in_domain_count_total)
            )
        )
        print(
            "total sites in domains: {}, expected in domains: {}".format(
                total_in_domains, round(expected_in_domain_count_total)
            )
        )
        outf.write(
            (
                "total expected rate of edit in domain(in domains/in all protein): {},total results: {}\n".format(
                    round(expected_in_domain_rate_total, 4),
                    round(in_domain_rate_total, 4),
                )
            )
        )
        print(
            "total expected rate of edit in domain(in domains/in all protein): {},total results: {}".format(
                round(expected_in_domain_rate_total, 4), round(in_domain_rate_total, 4)
            )
        )
        # summerize editing in 4 binding amino acid options
        outf.write(
            "fourth pattern:\nresidues in positions[-1, +2, +3, +6] : count:"
            + str(total_fourth)
        )
        print(
            "fourth pattern:\nresidues in positions[-1, +2, +3, +6] : count:",
