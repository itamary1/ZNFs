...
with open(path_As,'r') as A_sites:
    DictReader_obj = csv.DictReader(A_sites, delimiter="\t",fieldnames=["chrom", 'positions', 'strand'])
    for site in DictReader_obj:
        site['positions']=int(site['positions'])
        # find genes symbol
        site_genes_symb=genes_symb[(genes_symb['chrom'] == site['chrom']) & (genes_symb['strand'] == site['strand']) & (genes_symb['cdsStart'] <= site['positions']) & (genes_symb['cdsEnd'] >= site['positions'])]
