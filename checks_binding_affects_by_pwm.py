...
def compare_one_result(origial_prot_fasta_path,new_prot_fasta_path, suffix):
    ...
    for i in range(len(orig_pwm)):
        ...
        if orig_pwm[i].startswith(">"):
            orig_list=[orig_pwm[i+1].split(" ")[:-1],orig_pwm[i+2].split(" ")[:-1],orig_pwm[i+3].split(" ")[:-1],orig_pwm[i+4].split(" ")[:-1]]
            new_list=[new_pwm[i+1].split(" ")[:-1],new_pwm[i+2].split(" ")[:-1],new_pwm[i+3].split(" ")[:-1],new_pwm[i+4].split(" ")[:-1]]
...
def compare_pwm_result(origial_prot_fasta_path, new_prot_fasta_path):
    poll_diff = compare_one_result(origial_prot_fasta_path,new_prot_fasta_path, '_pol.pwm')
    lin_diff = compare_one_result(origial_prot_fasta_path,new_prot_fasta_path, '_lin.pwm')
...
if __name__ == '__main__':
    ...
    diffs_counters_pol = []
    diffs_counters_lin = []
    for index, gene in genes_all_changes.iterrows():
        gene_pol_diff=[]
        gene_lin_diff=[]
        gene_count_pol = 0
        gene_count_lin = 0
    ...
    # write fasta for every seq
        ...
        for mutation in mut_list:
            ...
            assert seq[int(mutation[1:-1])-1] == mutation[0], "ERROR amino place" + mutation + gene['gene_name']
    ...
    out_path="/private8/Projects/itamar/ZNF/Sra_GSE73211_35_samples/REDI/curated/domain_editing/finel_table_above_40_genes_pwm_result.csv"
